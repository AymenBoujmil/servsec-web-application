...
import { getContacts } from "src/actions/contacts";
...
function ListCat() {
  ...
  useEffect(() => {
    api.fetchContact().then((res) => {
      setContacts(res.data);
      setLoading(false);
    });
  }, []);
  let rows;
  if (!loading) rows = contacts;
  ...
  const EnhancedTableToolbar = (props) => {
    ...
    const handleDelete = (e) => {
      e.preventDefault();
      dispatch(deleteCategoory(numSelected));
    };

    const handleupdate = (e) => {
      e.preventDefault();
      history.push(`updateCategory/${numSelected}`);
    };
